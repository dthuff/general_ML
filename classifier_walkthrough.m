...
[ svm_X, svm_Y, svm_T, svm_AUC ] = train_svm( n_features, labels,...
...
figure();
plot(svm_X(:,1)', svm_Y(:,1)', 'k', 'LineWidth', 1); 
hold on; box on; grid on;

% pick some indices for plotting errorbars at
some_inds = round(1:size(svm_X,1)/20:size(svm_X,1));

errorbar(svm_X(some_inds,1), svm_Y(some_inds,1),...
        abs(svm_Y(some_inds,1)-svm_Y(some_inds,2)),...
        abs(svm_Y(some_inds,1)-svm_Y(some_inds,3)),...
        abs(svm_X(some_inds,1)-svm_X(some_inds,2)),...
        abs(svm_X(some_inds,1)-svm_X(some_inds,3)),...
        'color','k','linestyle','-', 'linewidth',1);
xlim([0,1]);
ylim([0,1]);
xlabel('1-Specificity','fontsize',14)
ylabel('Sensitivity','fontsize',14)
mean_auc_str = num2str(svm_AUC(1),2);
sd_auc_str = num2str(abs(svm_AUC(1)-svm_AUC(2)),2);
